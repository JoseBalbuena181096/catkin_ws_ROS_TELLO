...
class FollowFaceBebop:
    def __init__(self):
        self.bridge = CvBridge()
        self.pub_cmd_vel = rospy.Publisher('/bebop/cmd_vel', Twist, queue_size=10)
        ...
        self.image_reduction = rospy.get_param("/followFaceBebop/imageReduction")
    ...
    def control_flight(self):
        ...
        if self.run == 1:
            if abs(error_x) < 10 and abs(error_y) < 10:
                ...
                signal_angle_z = 0.3*(error_x)/self.width_by_2
                rospy.loginfo(f'signal control angular axis z {signal_angle_z}')
                signal_linear_z = 0.2 * (error_y) / self.height_by_2
                rospy.loginfo(f'signal control linear axis z {signal_linear_z}')

                self.vel_msg.linear.x = 0.0
                self.vel_msg.linear.y = round(signal_angle_z, 2)
...
def main():
    rospy.init_node('followFaceBebop', anonymous=True)
    followFaceBebop = FollowFaceBebop()
    try:
        rospy.spin()
